...
function tryToDecryptStoredEncrypted() {
	...
	for ( let i = 0; i < vm.$store.state.delta.downloaded.encrypted.length; ++i ) {
		const decrypted =  Decryptor( vm.$store.state.delta.downloaded.encrypted[ i ] );
		if ( decrypted ) {
	...
	for ( let i = 0; i < success_decrypted_indexes.length; ++i ) {
		vm.$store.state.delta.downloaded.encrypted.splice( success_decrypted_indexes[ i ] , 1 );
	}
